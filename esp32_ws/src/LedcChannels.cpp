...
bool LedcChannels::acquireSpecific(int channel) {
    ...
    channelMask |= mask;
    MLOG_INFO("LedcChannels: Acquired channel %d", channel);
    return true;
}
...
int LedcChannels::acquireFree() {
    for (int channel = 0; channel < 16; ++channel) {
        ...
        if (!(channelMask & mask)) {
            channelMask |= mask;
            MLOG_INFO("LedcChannels: Acquired free channel %d", channel);
            return channel;
        }
    }
...
void LedcChannels::release(int channel) {
    ...
    channelMask &= ~mask;
    MLOG_INFO("LedcChannels: Released channel %d", channel);
}
