...
PwdDevice::PwdDevice(const String &id, const String &name)
    : Device(id, "pwddevice"),
      _pin(-1),
      _pwmChannel(0),
      _frequency(5000),
...
bool PwdDevice::setupMotor(int pin, int pwmChannel, uint32_t frequency, uint8_t resolutionBits)
{
    if (pin < 0)
    {
        MLOG_ERROR("PwdDevice [%s]: Invalid pin %d. Pin must be >= 0.", _id.c_str(), pin);
        _isSetup = false;
        _pin = -1;
        return false;
    }
    ...
    {
        MLOG_ERROR("PwdDevice [%s]: Invalid PWM channel %d. Must be 0 or 1.", _id.c_str(), pwmChannel);
        _isSetup = false;
        return false;
    }
    ...
    if (frequency == 0)
    {
        MLOG_WARN("PwdDevice [%s]: Frequency cannot be 0. Falling back to 5000 Hz.", _id.c_str());
        frequency = 5000;
    }

    if (resolutionBits < 1 || resolutionBits > 16)
    {
        MLOG_WARN("PwdDevice [%s]: Resolution %d out of range. Clamping between 1 and 16.", _id.c_str(), resolutionBits);
    ...
    MLOG_INFO("PwdDevice [%s]: Setup - pin:%d, channel:%d, freq:%d Hz, resolution:%d bits",
...
bool PwdDevice::configureMCPWM()
{
    if (_pin < 0)
    {
        MLOG_WARN("PwdDevice [%s]: Cannot configure MCPWM without a valid pin.", _id.c_str());
        _isSetup = false;
        return false;
    }
    ...
    if (gpioErr != ESP_OK)
    {
        MLOG_ERROR("PwdDevice [%s]: Failed to initialize MCPWM GPIO: %s", _id.c_str(), esp_err_to_name(gpioErr));
        _isSetup = false;
        return false;
    }
    ...
    if (err != ESP_OK)
    {
        MLOG_ERROR("PwdDevice [%s]: MCPWM initialization failed: %s", _id.c_str(), esp_err_to_name(err));
        _isSetup = false;
        return false;
    }

    _isSetup = true;
    MLOG_INFO("PwdDevice [%s]: MCPWM configured successfully on pin %d", _id.c_str(), _pin);
    return true;
}

void PwdDevice::setDutyCycle(float dutyCycle, bool notifyChange)
{
    if (!_isSetup)
    {
        MLOG_ERROR("PwdDevice [%s]: Not setup. Call setupMotor() first.", _id.c_str());
        return;
    }
    ...
    if (err != ESP_OK)
    {
        MLOG_ERROR("PwdDevice [%s]: Failed to set duty cycle: %s", _id.c_str(), esp_err_to_name(err));
        return;
    }

    mcpwm_set_duty_type(_mcpwmUnit, _mcpwmTimer, MCPWM_OPR_A, MCPWM_DUTY_MODE_0);

    MLOG_INFO("PwdDevice [%s]: Duty cycle set to %.1f%%", _id.c_str(), dutyCycle);
...
void PwdDevice::setDutyCycleAnimated(float dutyCycle, uint32_t durationMs)
{
    if (!_isSetup)
    {
        MLOG_ERROR("PwdDevice [%s]: Not setup. Call setupMotor() first.", _id.c_str());
    ...
    MLOG_INFO("PwdDevice [%s]: Starting animated transition from %.1f%% to %.1f%% over %dms",
...
{
    setDutyCycle(0.0);
    MLOG_INFO("PwdDevice [%s]: Device stopped", _id.c_str());
}

void PwdDevice::setup()
{
    Device::setup();
...
bool PwdDevice::control(const String &action, JsonObject *args)
{
    if (action == "setDutyCycle")
    {
        if (!args || !(*args)["value"].is<float>())
        {
            MLOG_ERROR("PwdDevice [%s]: Invalid 'setDutyCycle' payload", _id.c_str());
            return false;
        }
    ...
    else if (action == "setup")
    {
        if (!args)
        {
            MLOG_ERROR("PwdDevice [%s]: 'setup' requires parameters", _id.c_str());
            return false;
        }
    ...
    {
        MLOG_WARN("PwdDevice [%s]: Unknown action: %s", _id.c_str(), action.c_str());
        return false;
    }
}

String PwdDevice::getState()
{
    JsonDocument doc;
...
String PwdDevice::getConfig() const
{
    JsonDocument config;
...
void PwdDevice::setConfig(JsonObject *config)
{
    Device::setConfig(config);

    if (!config)
    {
        MLOG_WARN("PwdDevice [%s]: Null config provided", _id.c_str());
        return;
    }
...
void PwdDevice::updateAnimation()
{
    if (!_isAnimating)
        return;
    ...
    if (elapsed >= _animationDuration)
    {
        ...
        MLOG_INFO("PwdDevice [%s]: Animation complete, final duty cycle: %.1f%%",
    ...
    if (err != ESP_OK)
    {
        MLOG_ERROR("PwdDevice [%s]: Failed to set animated duty cycle: %s", _id.c_str(), esp_err_to_name(err));
        _isAnimating = false;
        return;
    }
...
float PwdDevice::easeInOutQuad(float t)
{
    if (t < 0.0f)
        t = 0.0f;
    if (t > 1.0f)
...
std::vector<int> PwdDevice::getPins() const
{
    if (_pin < 0)
    {
        return {};
