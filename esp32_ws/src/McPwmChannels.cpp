...
bool McPwmChannels::acquireSpecific(int channel) {
    ...
    channelMask |= mask;
    MLOG_INFO("McPwmChannels: Acquired channel %d", channel);
    return true;
}
...
int McPwmChannels::acquireFree() {
    for (int channel = 0; channel < MCPWM_SIGNAL_COUNT; ++channel) {
        ...
        if (!(channelMask & mask)) {
            channelMask |= mask;
            MLOG_INFO("McPwmChannels: Acquired free channel %d", channel);
            return channel;
        }
    }
...
void McPwmChannels::release(int channel) {
    ...
    channelMask &= ~mask;
    MLOG_INFO("McPwmChannels: Released channel %d", channel);
}
...
mcpwm_io_signals_t McPwmChannels::getSignal(int channel) {
    switch (channel) {
        case MCPWM0_OUT0A: return MCPWM0A;
        case MCPWM0_OUT0B: return MCPWM0B;
        case MCPWM0_OUT1A: return MCPWM1A;
        case MCPWM0_OUT1B: return MCPWM1B;
        case MCPWM0_OUT2A: return MCPWM2A;
        case MCPWM0_OUT2B: return MCPWM2B;
        default:
