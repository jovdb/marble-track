...
interface PwdDeviceConfigProps {
  id: string;
  onClose: () => void;
}

export default function PwdDeviceConfig(props: PwdDeviceConfigProps) {
  const pwdDeviceStore = usePwdDevice(props.id);
  const device = () => pwdDeviceStore[0];
  const actions = pwdDeviceStore[1];

  const [name, setName] = createSignal(device()?.config?.name ?? device()?.id ?? "PWD Device");
